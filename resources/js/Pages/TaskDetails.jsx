...
import { X, PlusCircle, User, Users, Clock, CheckCircle, AlertCircle, Save, Trash2, Search, FilePlus, FileText, Calendar } from "lucide-react";
...
const TaskDetails = () => {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 flex items-center justify-center p-8">
        <div className="bg-white rounded-2xl shadow-xl p-12 text-center max-w-md">
          ...
          <p className="text-gray-600 text-lg mb-6">Task not found. Please return to the dashboard.</p>
          <button 
  ...
  const handleStatusChange = (status) => setLocalTaskData({ ...localTaskData, status });
  ...
  const handleUpdateTestingPoint = (id, updates) => {
    setLocalTaskData({
      ...localTaskData,
      testingPoints: localTaskData.testingPoints.map((p) => (p.id === id ? { ...p, ...updates } : p)),
    });
  };
  ...
  const handleSave = () => {
    router.put(`/task/${localTaskData.id}`, {
      ...
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <div className="flex items-center gap-2 ml-14">
                <span className={`px-4 py-1.5 rounded-full text-sm font-semibold border-2 ${getStatusColor(localTaskData.status)}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100 hover:shadow-md transition-shadow duration-200">
            ...
            <p className="text-lg font-semibold text-gray-800 ml-11">{localTaskData.developer}</p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100 hover:shadow-md transition-shadow duration-200">
            ...
            <p className="text-lg font-semibold text-gray-800 ml-11">{localTaskData.client}</p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100 hover:shadow-md transition-shadow duration-200">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-sm font-medium text-gray-500">Manager Timeline</span>
            </div>
            <p className="text-lg font-semibold text-gray-800 ml-11">{localTaskData.due}</p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm p-5 border border-gray-100 hover:shadow-md transition-shadow duration-200">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="text-sm font-medium text-gray-500">Developer Expected</span>
            </div>
            <p className="text-lg font-semibold text-gray-800 ml-11">{localTaskData.expectedTimeline || 'Not set'}</p>
          </div>
        </div>

        {/* Status Selector */}
        <div className="bg-white rounded-xl shadow-sm p-6 mb-6 border border-gray-100">
          <label className="block text-sm font-semibold text-gray-700 mb-3">Task Status</label>
          <select
            ...
            <div className="space-y-3">
              {['1.', '2.', '3.'].map((label, index) => (
                <div key={index} className="flex items-start gap-2">
                  <span className="font-bold text-sm text-red-700 self-start mt-3">{label}</span>
                  <input
                    type="text"
                    value={localTaskData.managerRemarks[index] || ''}
                    onChange={(e) => handleUpdateManagerRemark(index, e.target.value)}
        ...
        <div className="bg-gradient-to-br from-gray-50 to-gray-100 p-6 rounded-xl shadow-sm border border-gray-200 mb-6">
          ...
          <textarea
            ...
            onChange={(e) => setLocalTaskData({ ...localTaskData, overallRemark: e.target.value })}
          ...
          {filteredTestingPoints.length === 0 ? (
            <div className="text-center py-16 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
              ...
              <p className="text-gray-400 italic text-lg">
                {searchTerm ? `No testing data matches "${searchTerm}".` : 'No testing data available yet.'}
              </p>
            </div>
          ) : (
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-100">
                  {filteredTestingPoints.map((p, idx) => (
                    <tr key={p.id} className={`hover:bg-blue-50 transition-colors duration-150 ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                      ...
                      <td className="px-6 py-4">
                        <input
                          ...
                          onChange={(e) => handleUpdateTestingPoint(p.id, { managerRemark: e.target.value })}
        ...
        <div className="flex gap-4 mt-6">
          ...
          <button
            onClick={handleSave}
            className={`${canFinish ? 'flex-1' : 'w-full'} bg-gradient-to-r from-green-600 to-emerald-600 text-white py-4 rounded-xl font-bold hover:shadow-xl transition-all duration-200 flex items-center justify-center gap-3 text-lg`}
          >
